...
void NetIO::Accept()
{
	Acceptor.async_accept(AcceptSocket, [this](std::error_code ec) {
		mlog("NetIO::Accept - async_accept callback triggered, ec=%d (%s)\n", ec.value(), ec.message().c_str());
		if (!ec)
		{
			auto Endpoint = AcceptSocket.remote_endpoint();
			mlog("NetIO::Accept - Connection from %s:%d\n", Endpoint.address().to_string().c_str(), Endpoint.port());
			AcceptData Data{u32(Endpoint.address().to_v4().to_ulong()), Endpoint.port()};
			{
				...
				mlog("NetIO::Accept - Calling Callback\n");
				Callback(IOOperation::Accept, GetHandle(Conn), &Data);
				mlog("NetIO::Accept - Starting Read\n");
				Read(Conn);
			}
		}
		else
		{
			mlog("NetIO::Accept - Error: %s\n", ec.message().c_str());
		}
		Accept();
	});
}
...
bool NetIO::Create(int Port, CallbackType Callback, bool Reuse)
{
	...
	mlog("NetIO::Create - Creating acceptor on port %d\n", Port);

	// Create acceptor and start accepting BEFORE starting the IO thread
	tcp::endpoint LocalEndpoint{tcp::v4(), u16(Port)};
	Acceptor = tcp::acceptor(IOContext, LocalEndpoint, Reuse);
	Accept();

	mlog("NetIO::Create - Starting IO thread\n");

	auto ThreadProc = [this] {
		mlog("NetIO::Create - IO thread started\n");
		try {
			// Use work_guard to prevent run() from returning when idle
			...
			mlog("NetIO::Create - IO thread stopped\n");
		} catch (const std::exception& e) {
			mlog("NetIO::Create - IO thread exception: %s\n", e.what());
		}
	};
	for (size_t i = 0; i < 1; ++i)
		std::thread{ThreadProc}.detach();

	mlog("NetIO::Create - Done\n");
	return true;
}
